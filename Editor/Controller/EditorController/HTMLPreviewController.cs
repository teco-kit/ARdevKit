...
public class HTMLPreviewController
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// <summary>   The htmlView which we need to add Previewables. </summary>
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /// <summary>
    /// The scale of the previewhtmlView. we need this to scale the distance and the size of the elements.
    /// </summary>
    ...
    public HTMLPreviewController(EditorWindow ew)
    {
        if (ew == null)
            throw new ArgumentException("parameter ew was null.");

        this.ew = ew;
        this.htmlView = this.ew.HTMLPreview;
    ...
    ///
    /// <param name="p">    The htmlView to process. </param>
    ////////////////////////////////////////////////////////////////////////////////////////////////////

    [Obsolete("addPreviewable(IPreviewable p) : eache IPreviewable needs a Vector where the new"
                + "Previewable should sit in the htmlView you should use addPreviewable(IPreviewable"
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// <summary>
    /// add Trackable is the method for adding the trackable, each PreviewhtmlView can holding one
    /// Trackable.
    /// </summary>
    ...
    public void addPreviewable(IPreviewable currentElement, Vector3D v)
    {
        ...
        if (currentElement is AbstractTrackable && trackable == null)
        {
            ...
            center.Y = htmlView.Size.Height / 2;
            center.X = htmlView.Size.Width / 2;
            while (true)
            {
    ...
    public void removePreviewable(IPreviewable currentElement)
    {
        ...
        {
            this.htmlView.Controls.Remove(this.findBox((AbstractAugmentation)currentElement));
    ...
    /// <summary>
    /// Removes all Elements from the PreviewhtmlView and clears all references and delete the trackable from the list.
    /// </summary>
    private void removeAll()
    {
        this.htmlView.Controls.Clear();
        this.trackable = null;
    ...
    /// <summary>
    /// updates the preview htmlView.
    /// </summary>
    public void updatePreviewhtmlView()
    {
        this.htmlView.Controls.Clear();
        this.ew.project.Trackables.Add(trackable);
        updateElementCombobox(trackable);
        ...
        this.htmlView.ContextMenu = cm;
    }

    /// <summary>
    /// load the project with the identical index to the previewhtmlView 
    /// (the index is the index of the trackable list in project)
    /// </summary>
    /// <param name="index">The index.</param>
    public void reloadPreviewhtmlView(int index)
    {
        ...
        if (index < this.ew.project.Trackables.Count)
        {
            ...
            if (trackable != null)
            {
                this.addAllTohtmlView(this.ew.project.Trackables[index]);
            }
    ...
    /// <summary>
    /// Add all existent Objects of the trackable in the htmlView, this funktion is exists for change the trackable.
    /// </summary>
    /// <param name="trackable">The trackable.</param>
    private void addAllTohtmlView(AbstractTrackable trackable)
    {
    ...
    {
        this.htmlView.Controls.Remove(this.findBox(prev));
        if (prev is Chart)
        {
    ...
    public void addPictureBox(IPreviewable prev, Vector3D vector)
    {
        ...
        //creates the temporateBox with all variables, which'll be add than to the htmlView.
        PictureBox tempBox;
        ...
        this.htmlView.Controls.Add(tempBox);
    ...
    /// <summary>
    /// Searchs in the htmlView for the important PictureBox and gives this box back.
    /// </summary>
    /// <param name="prev">The previous.</param>
    /// <returns></returns>
    ...
    public PictureBox findBox(IPreviewable prev)
    {
        ...
        {
            foreach (Control comp in htmlView.Controls)
            {
    ...
    /// <summary>
    /// sets the currentElement in EditorWindow an marks the PictureBox in the PreviewhtmlView.
    /// </summary>
    /// <param name="currentElement">The current element.</param>
    public void setCurrentElement(IPreviewable currentElement)
    {
        ...
        if (currentElement != null)
        {
            if (this.ew.CurrentElement != currentElement)
            {
                ...
                foreach (Control comp in this.htmlView.Controls)
                {
    ...
    /// <summary>
    /// Recalculates the vector in dependency to htmlView.
    /// </summary>
    /// <param name="v">The v.</param>
    /// <returns>the new Vector in dependency to the htmlView</returns>
    private Vector3D recalculateVector(Vector3D v)
    {
        Vector3D result = new Vector3D(0, 0, 0);
        result.X = (htmlView.Width / 2 + v.X * 1.6 * scale);
        result.Y = (htmlView.Height / 2 - v.Y * 1.6 * scale);
        return result;
    }
    ...
    {
        Vector3D result = new Vector3D(0, 0, 0);
        result.X = (htmlView.Width / 2 + v.X);
        result.Y = (htmlView.Height / 2 - v.Y);
        return result;
    }
    ...
    /// <summary>
    /// Rescales the preview htmlView if the size was changed.
    /// </summary>

    public void rescalePreviewhtmlView()
    {
        int width = this.htmlView.Width;
        int height = this.htmlView.Height;
        ...
        foreach (AbstractTrackable trackable in this.ew.project.Trackables)
        {
            if (trackable != null)
            {
                ...
                foreach (AbstractAugmentation aug in trackable.Augmentations)
                {
                    if (aug is Chart)
                    {
                        ((Chart)aug).Positioning.Left = (int)((aug.Translation.X + htmlView.Width / 2));
                        ((Chart)aug).Positioning.Top = (int)((aug.Translation.Y + htmlView.Width / 2));
                    }
                }
            }
        }
    ...
    public void setCoordinates(IPreviewable prev, Vector3D newV)
    {
        ...
        if (prev is Chart)
        {
            ...
            Vector3D result = new Vector3D(0, 0, 0);
            result.X = (int)((newV.X - htmlView.Width / 2));
            result.Y = (int)((htmlView.Height / 2 - newV.Y));
            ((AbstractAugmentation)prev).Translation = result;
        }
        else
        {
            Vector3D result = new Vector3D(0, 0, 0);
            result.X = (int)((newV.X - htmlView.Width / 2) / scale / 1.6);
            result.Y = (int)((htmlView.Height / 2 - newV.Y) / scale / 1.6);
            ((AbstractAugmentation)prev).Translation = result;
        }
    }
    ...
    public void copy_augmentation(object sender, EventArgs e)
    {
        if (typeof(AbstractAugmentation).IsAssignableFrom(this.ew.CurrentElement.GetType()))
        {
            ...
            this.htmlView.ContextMenu.MenuItems[0].Enabled = true;
    ...
    public void paste_augmentation(object sender, EventArgs e)
    {
        if (this.trackable != null)
        {
            ...
            {
                Point p = this.htmlView.PointToClient(Cursor.Position);
    ...
    /// <summary>
    /// EventHandler for paste function. paste the object in the center of htmlView
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
    public void paste_augmentation_center(object sender, EventArgs e)
    {
        if (this.trackable != null)
        {
            ...
            {
                IPreviewable element = (IPreviewable)this.copy.Clone();
                this.addPreviewable(element, new Vector3D(this.htmlView.Width / 2, this.htmlView.Height / 2, 0));
    ...
    /// <summary>
    /// EventHandler to delete the PictureBox of the currentElement from the htmlView.
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
